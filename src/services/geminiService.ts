...
3. Sorularda her şık benzersiz ve özgün olsun. Tekrarlayan veya aynı anlama gelen ifadeler kullanma.
4. ŞIKLARIN BENZERSİZLİĞİ VE PERMÜTASYON YASAĞI: Seçeneklerde (şıklarda) yer alan öğe/bilgi listeleri birbirinin farklı sıralanmış versiyonları (permütasyonları) olmamalıdır! Örneğin; bir şık "araba, elma, kiraz" ise, diğer bir şık "kiraz, araba, elma" veya "elma, kiraz, araba" gibi aynı öğeleri içeren başka bir sıralama KESİNLİKLE olamaz. Bu durum soru kalitesini düşürür. Eğer birden fazla öğe içeren şıklar yazacaksan, her bir şıktaki öğe kombinasyonları birbirinden tamamen farklı olmalı ve şıklarda listelenen öğeler daima aynı mantıksal veya alfabetik/kronolojik sırayla yazılmalıdır.
5. ÖNCÜLLÜ (ROMA RAKAMLI) SORULARDA ŞIK KURALLARI: Eğer soru kökündeki metinde I, II, III gibi öncüller (roma rakamları) yer alıyorsa, şıklar (seçenekler) SADECE bu öncüllerin kombinasyonlarından (örn: 'Yalnız I', 'Yalnız II', 'I ve II', 'I, II ve III') veya 'Hiçbiri' ifadesinden oluşmalıdır. Şıklara öncüllerde yer almayan başka hiçbir kavramı, kelimeyi veya ismi (örn: 'Yalnızca Almanya', 'I ve İngiltere' gibi) KESİNLİKLE yazma!
`;
...
${excludeQuestionsInstruction}`;
  ...
  try {
    ...
    while (retries <= maxRetries) {
      try {
        console.log(`[Diagnostic] generateQuiz - Model: ${modelName}, Attempt ${retries + 1}/${maxRetries + 1}`);
        ...
        if (!textContent) {
          if (retries < maxRetries) {
            retries++;
            console.log(`Gemini returned an empty response. Retrying... (${retries}/${maxRetries})`);
            continue;
          }
        ...
        try {
          ...
          if (retries < maxRetries) {
            retries++;
            console.log(`Invalid JSON response from Gemini. Retrying... (${retries}/${maxRetries})`);
            continue;
          }
        ...
        if (!quiz || !quiz.questions || !Array.isArray(quiz.questions) || quiz.questions.length === 0) {
          if (retries < maxRetries) {
            retries++;
            console.log(`Quiz JSON contains no questions. Retrying... (${retries}/${maxRetries})`);
            continue;
          }
          throw new Error('API yanıtında sorular bulunamadı. Lütfen tekrar deneyin.');
        }

        // Validate each question & check duplicate/permuted options
        ...
        for (let index = 0; index < quiz.questions.length; index++) {
          ...
          if (!question.question_text) {
            console.warn(`Soru ${index + 1} metni boş.`);
            validationFailed = true;
            break;
          }

          // Programmatic duplicate/permuted options check
          if (hasDuplicateOrPermutedOptions(question.question_text, question.options)) {
            console.warn(`Soru ${index + 1} içerisinde benzer/mükerrer şıklar algılandı.`);
            validationFailed = true;
            break;
          }

          // Programmatic Roman numeral premise option validity check
          if (hasInvalidPremiseOptions(question.question_text, question.options)) {
            console.warn(`Soru ${index + 1} (öncüllü soru) içerisinde geçersiz şıklar algılandı.`);
            validationFailed = true;
            break;
          }
          ...
          validatedQuestions.push(cleanQuestionPlakas(question));
        }

        if (validationFailed) {
          if (retries < maxRetries) {
            retries++;
            console.log(`Validation failed (empty text or duplicate options). Retrying... (${retries}/${maxRetries})`);
            continue;
          }
          console.warn(`Validation failed after maximum retries. Proceeding with the generated questions to avoid failure.`);
        }

        quiz.questions = validatedQuestions;
        break; // Validation succeeded or out of retries; exit loop
      } catch (err: any) {
        if (err.message && (err.message.includes('API anahtarı geçersiz') || err.message.includes('PDF_EXPIRED') || err.message.includes('Geçersiz istek'))) {
          throw err;
        }
        if (retries < maxRetries) {
          retries++;
          console.warn(`Error in generateQuiz attempt ${retries}: ${err.message}. Retrying...`);
    ...
    if (!quiz) {
      throw new Error('Test oluşturulamadı. Lütfen tekrar deneyin.');
    }
...
ŞIKLARIN BENZERSİZLİĞİ VE PERMÜTASYON YASAĞI:
Seçeneklerde (şıklarda) yer alan öğe/bilgi listeleri birbirinin farklı sıralanmış versiyonları (permütasyonları) olmamalıdır! Örneğin; bir şık "araba, elma, kiraz" ise, diğer bir şık "kiraz, araba, elma" veya "elma, kiraz, araba" gibi aynı öğeleri içeren başka bir sıralama KESİNLİKLE olamaz. Bu durum soru kalitesini düşürür. Eğer birden fazla öğe içeren şıklar yazacaksan, her bir şıktaki öğe kombinasyonları birbirinden tamamen farklı olmalı ve şıklarda listelenen öğeler daima aynı mantıksal veya alfabetik/kronolojik sırayla yazılmalıdır.

ÖNCÜLLÜ (ROMA RAKAMLI) SORULARDA ŞIK KURALLARI:
Eğer soru kökündeki metinde I, II, III gibi öncüller (roma rakamları) yer alıyorsa, şıklar (seçenekler) SADECE bu öncüllerin kombinasyonlarından (örn: 'Yalnız I', 'Yalnız II', 'I ve II', 'I, II ve III') veya 'Hiçbiri' ifadesinden oluşmalıdır. Şıklara öncüllerde yer almayan başka hiçbir kavramı, kelimeyi veya ismi (örn: 'Yalnızca Almanya', 'I ve İngiltere' gibi) KESİNLİKLE yazma!
...
Lütfen JSON formatında ve tam olarak şu şemaya uygun bir nesne dön:
{
  ...
  let retries = 0;
  const maxRetries = 3;
  let similarQuestion: QuizQuestion | null = null;
  ...
  while (retries <= maxRetries) {
    try {
      ...
      if (!textResponse) {
        if (retries < maxRetries) {
          retries++;
          console.log(`generateSimilarQuestion got empty response. Retrying... (${retries}/${maxRetries})`);
          continue;
        }
        throw new Error('Yapay zeka geçerli bir soru döndüremedi.');
      }

      const parsedQuestion = JSON.parse(textResponse) as QuizQuestion;

      if (!parsedQuestion.question_text) {
        if (retries < maxRetries) {
          retries++;
          console.log(`generateSimilarQuestion got empty question text. Retrying... (${retries}/${maxRetries})`);
          continue;
        }
        throw new Error('Yapay zeka geçerli bir soru döndüremedi.');
      }
      ...
      let validationFailed = false;
      if (hasDuplicateOrPermutedOptions(parsedQuestion.question_text, parsedQuestion.options)) {
        console.log(`generateSimilarQuestion validation failed (duplicate options).`);
        validationFailed = true;
      } else if (hasInvalidPremiseOptions(parsedQuestion.question_text, parsedQuestion.options)) {
        console.log(`generateSimilarQuestion validation failed (invalid premise options).`);
        validationFailed = true;
      }
      ...
      if (validationFailed) {
        if (retries < maxRetries) {
          retries++;
          console.log(`generateSimilarQuestion validation failed. Retrying... (${retries}/${maxRetries})`);
          continue;
        }
        console.warn(`generateSimilarQuestion validation failed after maximum retries. Proceeding anyway.`);
      }

      similarQuestion = cleanQuestionPlakas(parsedQuestion);
      break;
    } catch (err: any) {
      if (err.message && (err.message.includes('API anahtarı geçersiz') || err.message.includes('PDF_EXPIRED'))) {
        throw err;
      }
      if (retries < maxRetries) {
        retries++;
        console.warn(`Error in generateSimilarQuestion attempt ${retries}: ${err.message}. Retrying...`);
        await new Promise(resolve => setTimeout(resolve, 1000));
        continue;
      }
      throw err;
  ...
  if (!similarQuestion) {
    throw new Error('Benzer soru oluşturulamadı. Lütfen tekrar deneyin.');
  }
...
/**
 * Normalizes an option string for permutation/duplicate detection.
 */
function normalizeOptionForComparison(opt: string): string {
  if (!opt) return '';

  // 1. Lowercase and trim
  let text = opt.toLowerCase().trim();

  // 2. Remove common Turkish conjunctions and punctuation
  text = text.replace(/\s+(?:ve|veya|ya\s+da|ile)\s+/gi, ',');
  text = text.replace(/;/g, ',');
  text = text.replace(/[\.\-\–\—\>]/g, ' '); // replace dots, dashes, and arrows with space

  // 3. Split by comma or whitespace into individual items
...
/**
 * Checks if a question has duplicate or permuted options.
 */
export function hasDuplicateOrPermutedOptions(questionText: string, options: Record<string, string>): boolean {
  const lowerText = questionText.toLowerCase();

  // Exclude sequence/chronology questions where order of items/numerals matters.
  const sequenceKeywords = [
    'kronolojik', 'sıra', 'sıralanış', 'sırasıyla', 'hangi sırayla',
    'eskiden yeniye', 'geçmişten günümüze', 'büyükten küçüğe', 'küçükten büyüğe',
    'sıralaması', 'sıralama', 'tarihsel sıralaması', 'doğru sıralama'
  ];

  if (sequenceKeywords.some(kw => lowerText.includes(kw))) {
    return false;
  }

  // Exclude option formats with sequential arrows or dashes
  const seqChars = ['>', '→', '->', '—'];
  if (Object.values(options).some(opt => seqChars.some(char => opt.includes(char)))) {
    return false;
  }
  ...
  for (const key of optionKeys) {
    ...
    const normVal = normalizeOptionForComparison(rawVal);
    if (!normVal) continue;

    if (normalizedMap.has(normVal)) {
      console.log(`[Diagnostic] Duplicate option detected between key ${normalizedMap.get(normVal)} ("${options[normalizedMap.get(normVal) as any]}") and key ${key} ("${rawVal}"). Normalized: "${normVal}"`);
      return true;
    }
    normalizedMap.set(normVal, key);
  }
...
/**
 * Checks if a question has invalid option values for Roman numeral premise questions.
 * Valid options in such questions should only consist of Roman numerals and allowed keywords (e.g. "Yalnız I", "I ve II", "Hiçbiri").
 */
export function hasInvalidPremiseOptions(questionText: string, options: Record<string, string>): boolean {
  const hasRomanNumerals = 
    (questionText.includes('I.') && questionText.includes('II.')) || 
    (questionText.includes('(I)') && questionText.includes('(II)'));
    
  if (!hasRomanNumerals) {
    return false; // Skip if it's not a Roman numeral premise question
  }
  ...
  for (const key of optionKeys) {
    ...
    let cleaned = rawVal.toLowerCase().trim();
    
    // Remove allowed roman numerals
    cleaned = cleaned.replace(/\b(?:i|ii|iii|iv|v)\b/g, '');
    
    // Remove allowed Turkish conjunctions and keywords
    const allowedWords = [
      'yalnız', 'yalnızca', 've', 'veya', 'ile', 'de', 'da', 
      'hiçbiri', 'hepsi', 'tümü', 'ortak', 'yalnızca biri', 
      'yalnızca ikisi', 'üçü de', 'yalnız biri', 'yalnız ikisi', 'sadece'
    ];
    ...
    if (cleaned.length > 0) {
      console.log(`[Diagnostic] Invalid premise option value detected for key ${key}: "${rawVal}". Remaining: "${cleaned}"`);
      return true;
    }
  }
