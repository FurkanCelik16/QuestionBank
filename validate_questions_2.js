...
Lütfen JSON objelerinin sonuna trailing comma (sondaki virgül) KOYMA.
...
${geographyMapInstruction}`;
  ...
  try {
    ...
    if (text) {
      ...
      // Cleanup trailing commas just like geminiService does now
      text = text.replace(/,\s*}/g, '}').replace(/,\s*]/g, ']');

      try {
        const parsed = JSON.parse(text);
        console.log("JSON parsing successful with regex cleanup.");
        let errors = [];
        for (const q of parsed.questions) {
          if (q.highlighted_province_ids.length > 0) {
             const opts = Object.values(q.options).join(" ");
             if (opts.match(/\b(34|06|61|35|07|I|II|III|IV|V)\b/i) && q.question_text.includes("harit")) {
                errors.push(`Q${q.id} contains invalid text in options for map question: ${opts}`);
             }
             if (q.question_text.match(/numara|işaretli yerlerden hangisi/i)) {
                errors.push(`Q${q.id} uses numbered places language: ${q.question_text}`);
             }
          }
        }
        if (errors.length > 0) console.log("ERRORS:", errors);
        else console.log("ALL LOGIC CHECKS PASSED!");
      } catch (e) {
